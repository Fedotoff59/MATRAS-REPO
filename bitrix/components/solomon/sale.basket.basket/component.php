...
if (strlen($_REQUEST["BasketRefresh"]) > 0 || strlen($_REQUEST["BasketOrder"]) > 0 || strlen($_REQUEST["DeleteAll"]) > 0)
{
	...
	while ($arBasketItems = $dbBasketItems->Fetch())
	{
		...
		if (($deleteTmp == "Y" && in_array("DELETE", $arParams["COLUMNS_LIST"])) || strlen($_REQUEST["DeleteAll"]) > 0)
		{
...
while ($arItems = $dbBasketItems->GetNext())
{
	...
	$arItems["WEIGHT_FORMATED"] = roundEx(DoubleVal($arItems["WEIGHT"]/$arParams["WEIGHT_KOEF"]), SALE_VALUE_PRECISION)." ".$arParams["WEIGHT_UNIT"];
        ...
        if ($arItems["MODULE"] == "catalog")
        {
            $dbElement = CIBlockElement::GetByID($arItems["PRODUCT_ID"]);
