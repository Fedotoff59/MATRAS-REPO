...
<?
	...
	foreach($arResult["ITEMS"]["AnDelCanBuy"] as $arBasketItems)
	{
		?>
			<tr>
                            <td class="t_2<?if ($i%2==1):?> gr<?endif;?>"><a href="<?=$arBasketItems["DETAIL_PAGE_URL"]?>"><?=$arBasketItems["NAME"]?><?=$arBasketItems["PREVIEW_PICTURE"]?></a></td>
                            <td class="t_2<?if ($i%2==1):?> gr<?endif;?>">
                                <?if ($arBasketItems['PROPS']['FORMAT_SIZES'][0] != ''):?>
                                    <strong>Размер:</strong><br />
                                    <?=$arBasketItems['PROPS']['FORMAT_SIZES'][0]?><br />
                                <?endif?>
                                <?if ($arBasketItems['PROPS']['MATREBASE'] != ''):?>
                                    <strong>Основа:</strong><br />
                                    <?=$arBasketItems['PROPS']['MATREBASE']?>
                                <?endif?>
                            </td>
