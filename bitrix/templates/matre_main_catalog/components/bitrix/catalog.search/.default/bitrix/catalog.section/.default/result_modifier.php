...
function PriceFilter($arItems) {
    if (isset($_GET['max_price']) && isset($_GET['min_price']) && $_GET['max_price'] != '' && $_GET['min_price'] !=''):
    $intMaxPrice = IntVal($_GET['max_price']);
    $intMinPrice = IntVAl($_GET['min_price']);
...
function SizeFilter($arItems) {
    if (isset($_GET['size']) && ($_GET['size'] != 'Неважно')):
    $reqSize = $_GET['size'];
    foreach($arItems as $cell => $arElement):
        foreach($arElement["OFFERS"] as $elID => $arOffer):
